...
class Read extends Mccms_Controller {
    ...
    public function index($page=1) {
        ...
        if(!empty($arr[3])){
            ...
            //组装SQL数据
            $sql = 'select {field} from `'.Mc_SqlPrefix.'read` where uid='.$uid;
            ...
            $pagearr = get_page($total,$pagejs,$page,$pagenum,'user/read/index/[page]',$row); 
    ...
    public function book($page=1) {
        ...
        if($page == 0) $page = 1;
        $data = array();
        $data['mccms_title'] = '小说阅读记录 - '.Web_Name;
        $uid = (int)$this->cookie->get('user_id');
        $row = $this->mcdb->get_row_arr('user','*',array('id'=>$uid));
        //获取模版
        $str = load_file('user/book_read.html');
        //预先解析分页标签
        $pagejs = 1;
        preg_match_all('/{mccms:([\S]+)\s+(.*?page=\"([\S]+)\".*?)}([\s\S]+?){\/mccms:\1}/',$str,$arr);
        if(!empty($arr[3])){
            ...
            $per_page = (int)$arr[3][0];
            //组装SQL数据
            $sql = 'select {field} from '.Mc_SqlPrefix.'book_read where uid='.$uid;
            $sqlstr = $this->parser->mccms_sql($arr[1][0],$arr[2][0],$arr[0][0],$arr[4][0],$sql);
            //总数量
            $total = $this->mcdb->get_sql_nums($sqlstr);
            //总页数
            ...
            //解析分页
            $pagenum = getpagenum($str);
            $pagearr = get_page($total,$pagejs,$page,$pagenum,'user/read/book/[page]',$row); 
            $pagearr[] = $per_page;$pagearr[] = $total;$pagearr[] = $pagejs;$pagearr[] = $page;
            $str = getpagetpl($str,$pagearr);
        }
        //全局解析
