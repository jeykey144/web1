...
class Book extends Mccms_Controller {
    ...
    public function index($page=1) {
        ...
        //网站标题
        $data['mccms_title'] = '我的小说 - '.Web_Name;
        //当前数据
        foreach ($row as $key => $val){
            $data['author_'.$key] = $val;
        }
        $str = load_file('author/book.html');
        //预先解析分页标签
        $pagejs = 1;
        ...
        if(!empty($arr[3])){
            ...
            //组装SQL数据
            $sqlstr = 'select * from '.Mc_SqlPrefix.'book where uid='.$uid.' order by addtime desc';
            //总数量
            ...
            $pagearr = get_page($total,$pagejs,$page,$pagenum,'author/book/index/[page]',$row); 
    ...
    public function add() {
        ...
        //网站标题
        $data['mccms_title'] = '新增小说 - '.Web_Name;
        //作者数据
        foreach ($author as $key => $val) $data['author_'.$key] = $val;
        //模版
        $str = load_file('author/book_add.html');
        //全局解析
        $str = $this->parser->parse_string($str,$data,true);
        //会员数据
    ...
    public function info($id=0) {
        ...
        $id = (int)$id;
        if($id == 0) get_err();
        $book = $this->mcdb->get_row_arr('book','*',array('id'=>$id,'uid'=>$uid));
        if(!$book) get_err();
        $data = array();
        $author = $this->mcdb->get_row_arr('user','*',array('id'=>$uid));
        //网站标题
        ...
        foreach ($author as $key => $val) $data['author_'.$key] = $val;
        //模版
        $str = load_file('author/book_info.html');
        //全局解析
        $str = $this->parser->parse_string($str,$data,true);
        //小说数据
    ...
    public function save() {
        ...
        if(empty($data['tags'])) get_json('主题标签不能为空~！');
        if(empty($data['text'])) get_json('一句话介绍不能为空~！');
        //新增
        if($id == 0){
            //判断五分钟内更新数量
            $time = time()-300;
            $mnum = $this->mcdb->get_nums('book',array('uid'=>$uid,'addtime>'=>$time));
            if($mnum > 3) get_json('系统检测到你有非法暴库行为~！');
            //判断小说标题相同
            $row = $this->mcdb->get_row_arr('book','id',array('name'=>$data['name']));
            if($row) get_json('存在相同名字的小说，请更改小说名字');
            //英文别名
            ...
            $url = links('author','bookchapter/add',$id);
        }else{
            $data['msg'] = '';
            $data['yid'] = 1;
            $this->mcdb->get_update('book',$id,$data);
            $url = links('author','book');
        }
    ...
    public function uppic() {
        ...
        //判断权限
        if($mid > 0){
            $muid = (int)getzd('book','uid',$mid);
            if($muid != $uid) get_json('没有操作权限!!!');
        }

        $cof['upload_path'] = FCPATH.Annex_Dir.'/book/'.get_str_date(Annex_Path).'/';
        ...
        if(!$this->upload->do_upload('image')){
            ...
            //更新数据库，进入待审核
            if($mid > 0){
                $this->mcdb->get_update('book',$mid,array($zd=>$img_file,'yid'=>1));
            }
            //返回
    ...
    public function del() {
        if(!$this->users->author(1)) get_json('登陆超时!!!');
        $id = (int)$this->input->post('id');
        if($id == 0) get_json('小说ID为空');
        $uid = (int)$this->cookie->get('user_id');
        $this->load->model('novel');
        $muid = (int)getzd('book','uid',$id);
